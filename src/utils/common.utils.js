...
exports.comparePassword = async (userPass,dbPass) => {
 ...
 exports.generateAccessToken = (userId,email,created_at) => {
    return  jwt.sign({ user_id: userId,created_at:created_at,email:email}, process.env.AUTH_SECRET_JWT, { expiresIn: '2d' }); 
};

exports.generateRefreshToken = (userId,email,created_at) => {
    return  jwt.sign({ user_id: userId,created_at:created_at,email:email}, process.env.AUTH_SECRET_JWT, { expiresIn: '30d' }); 
};
