...
class AuthRepository {
    ...
    userLogin = async (email, pass, is_register = false) => {
        ...
        if (!user) {
           ...
           return structureResponse([], false, "Email not registered");
        }
           ...
           if (!isMatch) {
            // throw new InvalidCredentialsException('Incorrect password');
             return structureResponse([], false, "Incorrect password");
           }
      ...
      if(affectedRows<=0){
        // throw new UpdateFailedException("AuthToken Updation Failed.");
         return structureResponse([], false, "AuthToken Updation Failed.");
      }
        let message = "";
        let responseBody = "";
        if (is_register){ 
